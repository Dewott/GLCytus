...
public class AdvancedGLRenderer extends Renderer {
	...
	public void finish(){
	    ...
	    for(Texture2D tex:textures){
		    if(i<tex.rect.layer){
				...
				gl.glTexImage2D(GL_TEXTURE_2D,0,GL_RGBA4,textureSize,textureSize,0,GL_RGBA,GL_UNSIGNED_BYTE,null);
				gl.glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MIN_FILTER,GL_LINEAR);
				gl.glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MAG_FILTER,GL_LINEAR);
				gl.glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_WRAP_S,GL_CLAMP_TO_EDGE);
				gl.glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_WRAP_T,GL_CLAMP_TO_EDGE);
			}
		    gl.glTexSubImage2D(GL_TEXTURE_2D,0,tex.rect.x,tex.rect.y,tex.rect.w,tex.rect.h,tex.data.getPixelFormat(),tex.data.getPixelType(),tex.data.getBuffer());
		}
		
		/*gl.glActiveTexture(GL_TEXTURE0);
		ByteBuffer buf=ByteBuffer.allocateDirect(67108864).order(ByteOrder.nativeOrder());
		gl.glGetTexImage(GL_TEXTURE_2D,0,GL_RGBA,GL_UNSIGNED_BYTE,buf);
		TextureData data = new TextureData(GLProfile.getDefault(),GL_RGBA,4096,4096,0,GL_RGBA,GL_UNSIGNED_BYTE,false,false,false,buf,null);
		try{
		TextureIO.write(data,new java.io.File("out.png"));
		}catch(Exception e){}*/
	
		int tex_loc = gl.glGetUniformLocation(shaderProgram, "tex");
		/*//loc(tex[i])=loc(tex[0])+i
		for(i=0;i<n;i++)
	...
	public void flushTaskQueue() {
		...
		for(int i=0;i<=drawCalls;i++){
		    if(i%2==0) gl.glBlendFunc(GL_SRC_ALPHA,GL_ONE_MINUS_SRC_ALPHA);
			else gl.glBlendFuncSeparate(GL_SRC_ALPHA,GL_ONE,GL_ONE,GL_ONE);
