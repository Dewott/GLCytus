	...
	public void flushTaskQueue() {
		...
		while (remaining > 0) {
			...
			if (task.img == null) {
				...
				gl.glBegin(GL_QUADS);
				// Vertex 1
				gl.glVertex3d(task.dstPts[0], task.dstPts[1], -MAX_OBJECT_COUNT
						+ i);
				// Vertex 2
				gl.glVertex3d(task.dstPts[2], task.dstPts[3], -MAX_OBJECT_COUNT
						+ i);
				// Vertex 3
				gl.glVertex3d(task.dstPts[4], task.dstPts[5], -MAX_OBJECT_COUNT
						+ i);
				// Vertex 4
				gl.glVertex3d(task.dstPts[6], task.dstPts[7], -MAX_OBJECT_COUNT
						+ i);
				gl.glEnd();
				gl.glFlush();
				v[i] = false;
				...
				for (int j = 0; j < v.length; j++)
					if (taskQueue.get(j).img.texture.boundTextureObject == currentTexture) {
						...
						gl.glVertex3d(task.dstPts[0], task.dstPts[1],
								-MAX_OBJECT_COUNT + j);
						// Vertex 2
						gl.glTexCoord2d(task.texPts[2], task.texPts[3]);
						gl.glVertex3d(task.dstPts[2], task.dstPts[3],
								-MAX_OBJECT_COUNT + j);
						// Vertex 3
						gl.glTexCoord2d(task.texPts[4], task.texPts[5]);
						gl.glVertex3d(task.dstPts[4], task.dstPts[5],
								-MAX_OBJECT_COUNT + j);
						// Vertex 4
						gl.glTexCoord2d(task.texPts[6], task.texPts[7]);
						gl.glVertex3d(task.dstPts[6], task.dstPts[7],
								-MAX_OBJECT_COUNT + j);
						gl.glEnd();
						gl.glFlush();
						v[j] = false;
