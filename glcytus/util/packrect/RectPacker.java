...
public class RectPacker {
	private int maxWidth = 0, maxHeight = 0, padding = 2;
	public LinkedList<Rect> rects = new LinkedList<Rect>();
	...
	public RectPacker(int width, int height, int padding) {
		maxWidth = width;
		maxHeight = height;
	...
	public void doLayout() {
		...
		while (rects.size() > 0) {
			...
			for (int i = 0; i < sortedLines.size(); i++) {
				...
				if (!done) {
					...
					if (bestFit != null) {
						...
						putRect(bestFit, line.x, line.y);
						HorizontalLine newLine = new HorizontalLine(line.x + bestFit.w + padding, line.y,
								line.w - bestFit.w - padding);
						line.w = bestFit.w;
						line.y += bestFit.h + padding;
						lines.add(newLine);
						done = true;
				...
				if (!done) {
					...
					for (int j = 0; j < n; j++)
						for (int k = j + 1; k < n; k++)
							if (((line.y + rects.get(index + j).h <= maxHeight)
									...
									&& (rects.get(index + j).w + rects.get(index + k).w + padding == line.w))) {
					...
					if (bestScore > 0) {
						...
						putRect(bestFit2, line.x + bestFit1.w + padding, line.y);
						HorizontalLine newLine = new HorizontalLine(line.x + bestFit1.w + padding,
								line.y + bestFit2.h + padding, line.w - bestFit1.w - padding);
						line.y += bestFit1.h + padding;
						line.w = bestFit1.w;
						lines.add(newLine);
						done = true;
						break;
			...
			if (!done) {
				...
				topLine = new HorizontalLine(0, 0, maxWidth);
				lines.add(topLine);
				sortedLines.add(topLine);
			} else
				checkAndMerge();
	...
	private int subListIndex(int left, int right, int key) {
		...
		if (rects.get(mid).w > key)
			return subListIndex(mid + 1, right, key);
		else
			return subListIndex(left, mid, key);
	}

	private boolean leftFit(HorizontalLine current, int h) {
		int index = lines.indexOf(current);
		if (index == 0)
			return current.y + h == maxHeight;
		else
			return current.y + h == lines.get(index - 1).y;
	}
	...
	/*
	 * public static void main(String args[]) throws Exception{ Rect rects[]=new
	 * Rect[128*128]; RectPacker packer = new RectPacker(1024,1024); for(int
	 * i=0;i<rects.length;i++){ rects[i]=new Rect(8,8); packer.add(rects[i]); }
	 * packer.doLayout();
	 * 
	 * BufferedImage img = new
	 * BufferedImage(1024,1024,BufferedImage.TYPE_INT_RGB); Graphics g =
	 * img.getGraphics(); g.setColor(Color.RED); for(int
	 * i=0;i<rects.length;i++){
	 * g.drawRect(rects[i].x,rects[i].y,rects[i].w,rects[i].h); } g.dispose();
