...
public class Click extends Note {
	...
	public Click(NoteChartPlayer p, int id, double time, double x, double y) {
		...
		case 1:// Default
			...
			nact.addTransform(new Transform(Transform.ALPHA, Transform.LINEAR, ntime1, ntime1 + p.beat / 2, 0, 0.8));
			nact.addTransform(new Transform(Transform.SCALE, Transform.LINEAR, ntime1, time, 0.5, 1));
			break;

		case 2:// Grouped
			circle.addTransform(new Transform(Transform.ALPHA, Transform.LINEAR, ntime1, ntime1 + p.beat / 2, 0, 1));
			circle.addTransform(new Transform(Transform.SCALE, Transform.LINEAR, ntime2 - p.beat / 2, ntime2, 0.5, 1));
			circle.addTransform(new BeatTransform(ntime2, p.pshift, p.beat / 2));
			nearadd.addTransform(new Transform(Transform.SCALE, Transform.LINEAR, ntime2 - p.beat / 2, ntime2, 0.5, 1));
			nearadd.addTransform(new BeatTransform(ntime2, p.pshift, p.beat / 2));
			nact.addTransform(new Transform(Transform.ALPHA, Transform.LINEAR, ntime1, ntime1 + p.beat / 2, 0, 0.8));
			nact.addTransform(new Transform(Transform.SCALE, Transform.LINEAR, ntime2 - p.beat / 2, ntime2, 0.5, 1));
			break;

		default: // None
			circle.addTransform(new Transform(Transform.ALPHA, Transform.LINEAR, ntime1, ntime1 + p.beat / 2, 0, 0.8));
			circle.addTransform(new BeatTransform(ntime2, p.pshift, p.beat / 2));
			nearadd.addTransform(new BeatTransform(ntime2, p.pshift, p.beat / 2));
			nact.addTransform(new Transform(Transform.ALPHA, Transform.LINEAR, ntime1, ntime1 + p.beat / 2, 0, 0.8));
		}

		shadow = new Animation("beat_shadow", time - p.beat * 0.4, time + 1 / 3.0);
		shadow.moveTo(x, y);
		shadow.addTransform(
				new Transform(Transform.ALPHA, Transform.LINEAR, time - p.beat * 0.3, time, 0.8, 0, true, false));
		shadow.addTransform(
