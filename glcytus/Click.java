	...
	public Click(NoteChartPlayer p, int id, double time, double x, double y) {
		...
		case 1:// Default
			...
			nearadd.addTransform(new Transform(Transform.SCALE,
					Transform.LINEAR, ntime1, time, 0.5, 1));
			flash.addTransform(new Transform(Transform.SCALE, Transform.LINEAR,
					ntime1, time, 0.5, 1));
			break;

		case 2:// Grouped
			circle.addTransform(new Transform(Transform.SCALE,
					Transform.LINEAR, ntime2 - p.beat / 3, ntime2, 0.5, 1));
		...
		nearadd.addTransform(new Transform(Transform.ALPHA,
					Transform.LINEAR, time-p.beat/2, time-p.beat*0.4, 0, 1));
        flash.addTransform(new Transform(Transform.ALPHA, Transform.LINEAR,
					ntime1, ntime1 + p.beat / 2, 0, 1));
		
		shadow = new Animation("beat_shadow", time - p.beat*0.3,
				time + 1 / 3.0);
		shadow.moveTo(x, y);
		shadow.addTransform(new Transform(Transform.ALPHA, Transform.LINEAR,
				time - p.beat*0.3, time, 1, 0, true, false));
		shadow.addTransform(new Transform(Transform.SCALE, Transform.LINEAR,
				time - p.beat*0.3, time, 1, 2, false, false));
		shadow.play(p);
	}
